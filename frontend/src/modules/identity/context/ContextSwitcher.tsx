...
function countProfiles(options: ContextOptionsResponse | undefined): number {
  if (!options) return 0;
  let count = 0;
...
export function ContextSwitcher() {
  ...
  const { data: optionsRaw } = useQuery<ContextOptionsResponse>({
  ...
  const profileCount = useMemo(() => countProfiles(optionsRaw), [optionsRaw]);
  const hasSingleProfile = profileCount <= 1;
