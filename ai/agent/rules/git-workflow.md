...
8. git add ai/project/changes/CHANGE-TASK-XXX.md
...
During testing:
- Create test reports (`ai/project/tests/TEST-TASK-XXX.md`)
- Create bug tasks (`ai/project/tasks/BUG-XXX.md`)
...
3. If all PRD tasks are now TESTED:
   ...
   - Update CHANGELOG.md
   - Run `git diff ai/project/prd/PRD-*.md` — verify ONLY `status` and `updated` changed
...
4. Create failure report: `ai/project/failures/FAIL-TASK-XXX.md`
...
2. The hook reads `.agent-role` + `ai/agent/rules/access.json`
3. It validates:
...
See `ai/agent/rules/access.md` for the full rule set.
