...
### Window Metadata (modules/metadata/) — NEW PRD-004
| Module | Description |
|--------|-------------|
| `modules/metadata/` | Window metadata entities: SysTable, SysColumn, SysWindow, SysTab, SysWindowField, SysWindowAccess, SysMenu. Services provide CRUD for each entity. Used by `WindowDefinitionAssemblyService` and `WindowDataService` in `core/runtime/`. Table name prefix `sys_*`. See `backend-metadata-window.md`. |
|--------|-------|------------|
