...
paths:
  - ai/agent/rules/access.json
  - ai/agent/rules/access.md
...
5. If you need to see what files you can modify, read `ai/agent/rules/access.md`
...
| Commit blocked with "File not allowed" | You tried to stage a file that isn't in your allowed paths. See `ai/agent/rules/access.md` for what your role can modify. |
...
| Warning: "Migration files staged but no schema update" | You committed DB migrations but didn't update the corresponding `ai/project/schema/*.sql` file. |
...
| File | Role |
|------|------|
| `ai/agent/rules/access.json` | Machine-readable rules — maps each role to allowed paths and branches |
| `ai/agent/rules/access.md` | Human-readable version for agents to read at startup |
...
Shared paths that multiple agents can modify: `ai/agent/project-board.md`, `ai/agent/changelog.md`.
...
sequenceDiagram
  ...
  participant Rules as ai/agent/rules/access.json
  ...
  Hook->>Rules: Read ai/agent/rules/access.json
...
- `ai/agent/rules/access.json` — the single source of truth for permissions
