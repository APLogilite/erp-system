...
public interface SysTabRepository extends JpaRepository<SysTab, UUID> {
  ...
  List<SysTab> findByWindowIdAndParentLinkColumnIdIsNullOrderBySeqNoAsc(UUID windowId);
  List<SysTab> findByWindowIdAndParentLinkColumnIdIsNotNullOrderBySeqNoAsc(UUID windowId);
}
